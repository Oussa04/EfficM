...
public class TicketCommandeAdapter extends RecyclerView.Adapter<TicketCommandeAdapter.TicketHolder> {
...
//            });
    ...
    public TicketCommandeAdapter(Context mContext, TicketListener listener, List<DetailCommande> detailCommandeList, boolean showButtons) {
    ...
    public void onBindViewHolder(final TicketHolder holder, int position) {
        ...
        holder.productname.setText(ticket.getProductName() + " - x" + ticket.getQuantity());
        holder.singleprice.setText( String.format("%.3f",ticket.getPrice()) + " dt.");
        holder.total.setText(String.format("%.3f",ticket.getPrice() * ticket.getQuantity()) + " dt.");
