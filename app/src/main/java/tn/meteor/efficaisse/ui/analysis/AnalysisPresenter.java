...
public class AnalysisPresenter implements AnalysisContract.Presenter {
    ...
    AnalysisPresenter(AnalysisContract.View view) {
        this.view = view;
    }
    ...
    public void getPredictions() {
        ...
        for (Ingredient ingredient :
            ...
            predictions.add(getPredictionByIngredient(ingredient));

        }

        view.updateUI(predictions);

    }


    private Prediction getPredictionByIngredient(Ingredient ingredient) {
        ...
        DateFormat formatterDayOfTheWeek = new SimpleDateFormat("EEE");
        ...
        int count;
        if (((countMonth + 1.0) / (precByMonth.getCount() + 1.0)) < ((countDayOfTheWeek + 1.0) / (precDay.getCount() + 1.0))) {
            count = countMonth;
        ...
        if ((count + 1.0) / (prediction.getCount() + 1.0) > (countweek + 1.0) / (precWeek.getCount() + 1.0)) {
